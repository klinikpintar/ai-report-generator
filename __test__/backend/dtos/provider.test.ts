...
describe('Provider DTO Validation', () => {
  ...
  describe('GET Schema', () => {
    ...
    // ✅ Happy Path - Boolean Input Parameters
    test('should handle native boolean values for includeModels', () => {
      // Test kasus ketika input sudah berupa boolean
      const paramsWithTrueBoolean = {
        includeModels: true // Boolean asli, bukan string
      };

      const resultTrue = ProviderValidation.GET.parse(paramsWithTrueBoolean);
      expect(resultTrue.includeModels).toBe(true);
      expect(typeof resultTrue.includeModels).toBe('boolean');

      const paramsWithFalseBoolean = {
        includeModels: false // Boolean asli, bukan string
      };

      const resultFalse = ProviderValidation.GET.parse(paramsWithFalseBoolean);
      expect(resultFalse.includeModels).toBe(false);
      expect(typeof resultFalse.includeModels).toBe('boolean');
    });
  });
