...
describe("CRUD of Schema API (Using NextRequest)", () => {
  ...
  it('should return response 400 Bad Request for invalid platform codes', async () => {
    const invalidPlatformCodes = ['INVALID_PLATFORM'];

    const request = new NextRequest(
      new URL(
        `http://localhost/api/schema?platformCodes=${invalidPlatformCodes[0]}`
      ),
      {
        method: "GET",
      }
  ...
  it("should return 400 Bad Request for serviceIds other than UUID", async () => {
    const invalidServiceId = "invalid-service-id";

    const request = new NextRequest(
      new URL(
        `http://localhost/api/schema?serviceIds=${invalidServiceId}`
      ),
      {
        method: "GET",
      }
    ...
    expect(response.status).toBe(StatusCodes.BAD_REQUEST);
    expect(json.message).toBe("Service ID must be a valid UUID");
  })
