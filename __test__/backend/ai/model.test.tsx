...
// Mock untuk service methods
const mockGetAllProviders = jest.fn();
const mockGetActiveOrDefaultProvider = jest.fn();
const mockUpdateActiveModel = jest.fn();

jest.mock('@/app/(backend)/factories/serviceFactory', () => {
  // Gunakan object literal untuk mengembalikan mock
  return {
    ServiceFactory: {
      getProviderService: () => ({
        getAllProviders: mockGetAllProviders,
        getActiveOrDefaultProvider: mockGetActiveOrDefaultProvider,
        updateActiveModel: mockUpdateActiveModel,
      })
    }
  };
});
...
describe('AI Provider API - GET Models', () => {
  ...
  test("Should retrieve providers with models successfully", async () => {
    ...
    expect(mockGetAllProviders).toHaveBeenCalledWith(
  ...
  test("Should create default provider if no providers exist", async () => {
    ...
    mockGetActiveOrDefaultProvider.mockResolvedValue(defaultProvider);
    ...
    expect(mockGetActiveOrDefaultProvider).toHaveBeenCalled();
  });
  ...
  test("Should handle service errors during model fetch", async () => {
    // Mock service error
    mockGetAllProviders.mockRejectedValue(new Error('Database error'));
...
describe('AI Provider API - PATCH (Set Active Model)', () => {
  ...
  test("Should set active model successfully", async () => {
    ...
    mockUpdateActiveModel.mockResolvedValue(mockUpdatedProvider);
    ...
    expect(mockUpdateActiveModel).toHaveBeenCalledWith(
  ...
  test("Should handle service errors gracefully", async () => {
    ...
    // Mock service error
    mockUpdateActiveModel.mockRejectedValue(new Error('Service error'));
